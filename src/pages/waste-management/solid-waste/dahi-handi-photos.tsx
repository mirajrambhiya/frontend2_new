...
const dahiHandiPhotosCategories = [
    "2019", "2018", "2017", "2016", "2015", "2014", "2013", "2012", "2011", "2010"
];

const DahiHandiPhotos = () => {
    const [selectedCategory, setSelectedCategory] = useState(dahiHandiPhotosCategories[0]);
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const [selectedAlbum, setSelectedAlbum] = useState<any>(null);
    const [currentImageIndex, setCurrentImageIndex] = useState(0);

    const galleryData: Record<string, any[]> = {};
    dahiHandiPhotosCategories.forEach(cat => {
        galleryData[cat] = Array.from({ length: 8 }, (_, i) => ({
            id: i + 1,
            title: `Dahi Handi ${cat} - Album ${i + 1}`,
            date: cat,
            images: [`https://placehold.co/800x600?text=Dahi+Handi+${cat}+Image+${i + 1}`]
        }));
    });
        ...
        <div className="min-h-screen bg-[#FDFDFD] pb-20 font-family-helvetica text-[#333333]">
            ...
            <div className="w-full max-w-[1282px] mx-auto px-4 py-12">
                    ...
                    <div className="w-full max-w-[300px]">
                        ...
                        {isDropdownOpen && (
                            <div className="absolute top-full left-0 w-full mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-y-auto max-h-[400px] animate-in fade-in slide-in-from-top-2 duration-200">
                                {dahiHandiPhotosCategories.map((cat) => (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {galleryData[selectedCategory].map((item) => (
                        <GalleryCard key={item.id} images={item.images} title={item.title} date={item.date} onClick={() => openSlideshow(item)} />
                    ))}
                </div>
            </div>
            ...
            {selectedAlbum && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/95 backdrop-blur-md p-4" onClick={() => setSelectedAlbum(null)}>
                    <button onClick={() => setSelectedAlbum(null)} className="absolute top-8 right-8 p-3 text-white bg-white/10 hover:bg-white/20 rounded-full transition-all z-[1001] cursor-pointer"><X className="w-8 h-8" /></button>
                    <div className="flex items-center justify-center gap-2 max-h-[90vh] w-full max-w-7xl" onClick={(e) => e.stopPropagation()}>
                        <button onClick={handlePrevImage} className="cursor-pointer p-3 text-white/80 hover:text-white bg-white/10 rounded-full transition-all flex-shrink-0 z-[1001]"><ChevronLeft className="w-8 h-8 md:w-12 md:h-12" /></button>
                        <div className="relative shrink-0 flex items-center justify-center">
                            <img src={selectedAlbum.images[currentImageIndex]} alt="Slide" className="max-h-[85vh] max-w-[calc(100vw-140px)] object-contain rounded-lg shadow-2xl" />
                            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white/90 font-medium bg-black/60 px-4 py-1 rounded-full backdrop-blur-sm z-10">{currentImageIndex + 1} / {selectedAlbum.images.length}</div>
                        </div>
                        <button onClick={handleNextImage} className="cursor-pointer p-3 text-white/80 hover:text-white bg-white/10 rounded-full transition-all flex-shrink-0 z-[1001]"><ChevronRight className="w-8 h-8 md:w-12 md:h-12" /></button>
                    </div>
                </div>
            )}
        </div>
...
export default DahiHandiPhotos;
