...
const miscellaneousPhotosCategories = [
    "Cotton bag distribution by Aditya Thackeray"
];

const MiscellaneousPhotos = () => {
    const [selectedCategory, setSelectedCategory] = useState(miscellaneousPhotosCategories[0]);
    ...
    const galleryData: Record<string, any[]> = {
        "Cotton bag distribution by Aditya Thackeray": Array.from({ length: 6 }, (_, i) => ({
            id: i + 1,
            title: `Cotton bag distribution - Part ${i + 1}`,
            date: "2024",
            images: [
                `https://placehold.co/800x600?text=Cotton+Bag+Distribution+Image+${i + 1}`
            ]
        }))
    };
                    ...
                    <div className="relative w-full h-[200px] md:h-[250px] lg:h-[280px] rounded-t-[30px] md:rounded-t-[40px] lg:rounded-t-[48px] overflow-hidden">
                        ...
                        <div className="relative z-30 flex flex-col items-center justify-center h-full text-center px-4 md:px-6 mt-[-20px] md:mt-[-30px]">
                            <h1 className="text-[28px] md:text-[36px] lg:text-[40px] font-bold text-[#000000] tracking-tight leading-tight md:leading-none mb-4 md:mb-8">
                                Miscellaneous Programs - Photo Gallery
                            </h1>
                        </div>
                    ...
                    <div className="w-full max-w-[500px]">
                        ...
                        {isDropdownOpen && (
                            <div className="absolute top-full left-0 w-full mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                {miscellaneousPhotosCategories.map((cat) => (
                                    <button
                                        key={cat}
                                        onClick={() => {
...
export default MiscellaneousPhotos;
