...
const diwaliPhotosCategories = [
    "Diwali Festival Photos"
];

const DiwaliFestivalPhotos = () => {
    const [selectedCategory, setSelectedCategory] = useState(diwaliPhotosCategories[0]);
    ...
    const galleryData: Record<string, any[]> = {
        "Diwali Festival Photos": Array.from({ length: 12 }, (_, i) => ({
            id: i + 1,
            title: `Diwali Festival Photo ${i + 1}`,
            date: "2024",
            images: [
                `https://placehold.co/800x600?text=Diwali+Festival+Image+${i + 1}`
            ]
        }))
    };
                    ...
                    <div className="w-full max-w-[400px]">
                        ...
                        {isDropdownOpen && (
                            <div className="absolute top-full left-0 w-full mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                {diwaliPhotosCategories.map((cat) => (
                                    <button
                                        key={cat}
                                        onClick={() => {
...
export default DiwaliFestivalPhotos;
