...
const worldEnvDayPhotosCategories = [
    "2019",
    "2018",
    "2017",
    "2016",
...
const WorldEnvironmentDayPhotos = () => {
    const [selectedCategory, setSelectedCategory] = useState(worldEnvDayPhotosCategories[0]);
    ...
    worldEnvDayPhotosCategories.forEach(cat => {
        galleryData[cat] = Array.from({ length: 8 }, (_, i) => ({
            id: i + 1,
            title: `World Environment Day ${cat} - Album ${i + 1}`,
            date: cat,
            images: [
                `https://placehold.co/800x600?text=World+Environment+Day+${cat}+Image+${i + 1}`
            ]
        }));
    });
                    ...
                    <div className="w-full max-w-[300px]">
                        ...
                        {isDropdownOpen && (
                            <div className="absolute top-full left-0 w-full mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                {worldEnvDayPhotosCategories.map((cat) => (
                                    <button
                                        key={cat}
                                        onClick={() => {
...
export default WorldEnvironmentDayPhotos;
