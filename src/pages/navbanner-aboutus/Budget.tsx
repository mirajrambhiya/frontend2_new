...
// 9 colors that cycle for budget cards
const budgetColors = [
    "bg-green-500",
    "bg-cyan-500",
    "bg-red-400",
...
// Generate budget years from 2014-15 to 2025-26 (most recent first)
const generateBudgetCards = () => {
    ...
    for (let year = endYear; year >= startYear; year--) {
        ...
        const colorIndex = (endYear - year) % budgetColors.length;
        cards.push({
            id: year,
            year: yearString,
...
const budgetCards = generateBudgetCards();

// Budget Card component
const BudgetCard = ({ year, color }: { year: string; color: string }) => {
...
function Budget() {
    return (
        <div>
            <AboutUsBanner />
            <Banner bannerSection="budget" />

            <div className="max-w-6xl mx-auto px-4 md:px-8 py-4 md:py-8 font-family-helvetica">
                {/* Title */}
                <h1 className="text-2xl md:text-3xl font-semibold text-gray-800 mb-6 md:mb-8">Budget</h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 place-items-center">
                    {budgetCards.map((card) => (
                        <BudgetCard key={card.id} year={card.year} color={card.color} />
                    ))}
                </div>
            </div>
        </div>
